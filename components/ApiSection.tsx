...
const CodeLine: React.FC<{ number: number; line: string }> = ({ number, line }) => {
  ...
  return (
    <div>
      <span className="text-gray-500 dark:text-gray-500 w-8 inline-block text-right pr-4 select-none">{number}</span>
...
const ApiSection: React.FC<ApiSectionProps> = ({ onGitHubClick }) => {
  ...
  return (
    <section className="py-24">
      ...
      <div className="max-w-4xl mx-auto bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-800">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-gray-900 dark:text-white font-medium">Clean, Efficient, and Modern Code</h3>
          ...
          <div className="bg-gray-100 dark:bg-gray-800 p-1 rounded-md text-sm">
            <button 
              onClick={() => setLang('nextjs')}
              className={`px-3 py-1 rounded ${lang === 'nextjs' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'}`}
            >
              Next.js
            </button>
            <button 
              onClick={() => setLang('js')}
              className={`px-3 py-1 rounded ${lang === 'js' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'}`}
            >
              JavaScript
            </button>
          </div>
        ...
        <div className="bg-gray-100/50 dark:bg-black/20 rounded-lg p-4 font-mono text-sm text-gray-700 dark:text-gray-300 overflow-x-auto">
          <pre><code>
